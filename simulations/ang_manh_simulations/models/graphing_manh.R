...
d = read_csv("../series/series1/model_output/data_fixed1.csv",skip_empty_rows=TRUE) %>% 
  drop_na()
summary(d)
nrow(d)
...
d_low = d %>% 
  ...
  mutate(UtteranceType = case_when(
    ...
    Language == "Vietnamese" & Utterance == "pin blue and small + pin small and blue" ~ "redundant",
...
p <- ggplot(d_low, aes(x=Language,y=speakerProb)) +
  geom_bar(stat="identity",position=dodge) + 
  scale_fill_manual(values=c("#4287f5","#fff200"),name="") +
  facet_grid(cost~ContextType, labeller = label_both) + 
  ggtitle("Revised Graph 1st Scenario") +
  ylim(0,1) +
...
ggsave(file="../series/series1/model_output/low_variation_graphs",width=7,height=5)
# 
