...
runModel <- function(backend, engine, modelAndSemantics, cmd, states, utterances, alpha = 1, materialNoiseVal = 1, colorNoiseVal = 1, 
                     materialCost = 0, colorCost = 0, nounCost = 0) {
...
var model = extend(model(params), \n {states : %s, utterances : %s}) 
                 ", alpha, materialNoiseVal, colorNoiseVal, materialCost, colorCost, nounCost, toJSON(states), toJSON(utterances))
