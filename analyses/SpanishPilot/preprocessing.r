...
saveRDS(player_info, file = "../../data/SpanishPilot/rawPlayerInfo.rds")

## Option (b): read in the raw data from .rds (rather than querying the database)

player_info <- readRDS("../../data/SpanishPilot/rawPlayerInfo.rds")
...
length(unique((d_preManualTypoCorrection %>% filter(grepl("color|size", condition)))$gameId))
...
write_delim(data.frame(d_preManualTypoCorrection %>%
                         select(-target, -images, -listenerImages, -speakerImages,
                                -chat)), 
            "../../data/SpanishPilot/preManualTypoCorrection.tsv", delim="\t")
...
d <- read_delim("../../data/SpanishPilot/postManualTypoCorrection.tsv", delim = "\t") %>%
...
destinationFolder <- "../../data/SpanishPilot/dummy"
...
destinationFolder <- "../../data/SpanishPilot"
