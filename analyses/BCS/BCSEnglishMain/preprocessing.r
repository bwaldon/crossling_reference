...
write_delim(data.frame(d_preManualTypoCorrection %>%
                         select(-target, -images, -listenerImages, -speakerImages,
                                -chat)), 
            "../../data/BCS/BCSEnglishMain/preManualTypoCorrection.tsv", delim="\t")
...
d <- read_delim("../../../data/BCS/BCSEnglishMain/postManualTypoCorrection.tsv", delim = "\t") %>%
...
numOtherTrials/nrow(d) #19.8% of trials contained an "other" type of referring expression

# Get rid of all of those types of trials
d <- d %>%
  filter(!grepl("O", words))

destinationFolder <- "../../../data/BCS/BCSEnglishMain"
