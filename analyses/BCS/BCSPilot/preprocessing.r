...
test <- readRDS("../../../data/BCS/BCSPilot/rawPlayerInfo.rds")
...
write_delim(data.frame(d_preManualTypoCorrection %>%
                         select(-target, -images, -listenerImages, -speakerImages,
                                -chat)), 
            "../../../data/BCS/BCSPilot/preManualTypoCorrection.tsv", delim="\t")
...
d <- read_delim("../../../data/BCS/BCSPilot/postManualTypoCorrection.tsv", delim = "\t") %>%
  filter(condition %in% c("scenario1", "scenario2", "scenario3", "scenario4")) %>%
...
destinationFolder <- "../../../data/BCS/BCSPilot"
