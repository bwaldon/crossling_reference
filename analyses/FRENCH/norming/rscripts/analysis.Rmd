...
source("./analysisHelper.R")

#data sources for stimuli: colors and objects
wordListRaw = read.csv("../data_input/wordList.csv")
wordList <- wordListRaw %>%
  select(Item, Gender) %>% 
  unique()
colorList = read.csv("../data_input/colorList.csv")
colorList <- colorList[-c(2)]

#participant data
df = read.csv("../data_input/french_noun_norming-merged_2.csv", header = TRUE)
cf = read.csv("../data_input/french_color_norming-merged_2.csv", header = TRUE)
...
view(below)
view(above)
above <- apply(above,2,as.character)
write.csv(above, "../data_output/noun_final.csv")
nrow(below) #21
nrow(above) #83
