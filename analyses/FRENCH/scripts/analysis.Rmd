...
val <- getPercent("present")
label <- getLabel("present")

#appends most common responses and their percentages to the wordlist
results <- wordList %>% mutate(common = apply(wordList, 1, getLabel)) %>%
  mutate(percent = apply(wordList, 1, getPercent))
view(results)

#Filtering by which words reached an agreement of 70% or more
below <- results %>% filter(percent < 0.70)
above <- results %>% filter(percent >= 0.70)
view(below)
view(above)
nrow(below) #21
nrow(above) #83
