...
d = mutate(d, Redundant_Property = rawD$data$condition, .after = condition)
d = mutate(d, Sufficient_Property = ifelse(d$Redundant_Property == "color", "size", "color"), .before = condition)
d = mutate(d, redundant = (d$colorMentioned == T & d$Redundant_Property == "color") | (d$sizeMentioned == T & d$Redundant_Property == "size"), .after = Redundant_Property)
