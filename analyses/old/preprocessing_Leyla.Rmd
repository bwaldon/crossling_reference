...
ggsave(file="../graphs/accuracy.pdf",width=5,height=3)
```
...
TODO: manual correction + code 'other' modifiers (still not changed -from pd)
```{r manually_corrected, eval=FALSE, include=FALSE}
# tinfo = d %>% mutate(id = row_number()) %>% select(id,trialType) #write.table
