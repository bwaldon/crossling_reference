...
export default class AdminNewBatch extends React.Component {
  ...
  handleVisibilityChange = (event) => {
    const batchGroupVisibility = event.currentTarget.value;
    this.setState({
      batchGroupVisibility,
      batchGroupVisibility: batchGroupVisibility,
    });
  };

  handleBatchGroupNameChange = (event) => {
    const batchGroupName = event.currentTarget.value;
    this.setState({
      batchGroupName,
  ...
  handleNewBatch = (event) => {
    ...
    const params = { assignment, batchGroupVisibility, batchGroupName };

    if (batchGroupVisibility == "private") {
      if (!batchGroupName.match("^[A-Za-z0-9]+$") || batchGroupName == "") {
        AlertToaster.show({
          message:
            "Batch collection name must be nonempty and contain only letters/numbers.",
        });
        return;
      }
    }
  ...
  renderContent() {
    ...
    return (
      <form className="new-batch" onSubmit={this.handleNewBatch}>
        <div className={Classes.DIALOG_BODY}>
          <FormGroup label="Batch Visibility" labelFor="batchGroupVisibility">
            <HTMLSelect
              name="batchGroupVisibility"
              id="batchGroupVisibility"
              onChange={this.handleVisibilityChange}
              value={batchGroupVisibility}
            >
              {_.map(batchGroupVisibilityTypes, (name, key) => (
          ...
          {batchGroupVisibility == "private" ? (
            <FormGroup label="Batch Group Name" labelFor="batchGroupName">
              <input
                name="batchGroupName"
                id="batchGroupName"
                type="text"
                onChange={this.handleBatchGroupNameChange}
